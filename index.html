...
<head>
  ...
  <title>A Year With You — Phue</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    html,body{height:100%;margin:0;font-family:Poppins,system-ui,Arial;background:linear-gradient(180deg,var(--ivory),#fff7fb);color:var(--text);}
...
<body>
  ...
  <div class="hero">
    <div class="container">
      <h1>Happy Birthday, Phue 🌸</h1>
      <p>A little website that holds our year — made especially for you.</p>
      <button class="btn" id="startBtn">Start the Journey 💕</button>
      <p class="small">April 8 — today's your day ✨</p>
    </div>

    <!-- decorative hearts -->
    <div class="particle" style="top:10%;left:8%"><div class="heart"></div></div>
    <div class="particle" style="top:30%;right:10%;transform:scale(1.6);opacity:0.08"><div class="heart"></div></div>
    <div class="particle" style="bottom:10%;left:20%;transform:scale(1.2);opacity:0.07"><div class="heart"></div></div>
  </div>
  ...
  <div class="container">
    ...
    <section class="gallery" id="gallery">
      <h2 style="font-family:'Great Vibes',cursive;font-size:34px;margin:4px 0">Our Year in Moments</h2>
      <p class="small">Swipe or scroll — each photo holds a memory.</p>

      <div class="album" id="album">
        <!-- Replace the src values with your own photos. Add more .photo blocks if you like. -->
        <div class="photo">
          <img src="images/photo1.jpg" alt="photo 1">
          <div class="caption">That day we couldn't stop laughing</div>
        </div>
        <div class="photo">
          <img src="images/photo2.jpg" alt="photo 2">
          <div class="caption">A little rainy-day adventure</div>
        </div>
        <div class="photo">
          <img src="images/photo3.jpg" alt="photo 3">
          <div class="caption">When time felt slow and perfect</div>
        </div>
        <div class="photo">
          <img src="images/photo4.jpg" alt="photo 4">
          <div class="caption">Your smile lit up everything</div>
        </div>
        <div class="photo">
          <img src="images/photo5.jpg" alt="photo 5">
          <div class="caption">A quiet moment just for us</div>
        </div>
      </div>
    ...
    <section class="message">
      ...
      <p>Through every smile, every conversation, and all the small adventures — you made this year brighter. I wanted to give you something that holds those moments.</p>
    </section>

    <!-- The Ask -->
    <section class="ask">
      <h3>So... I have one question.</h3>
      <p style="margin:0 0 8px">Will you be my girlfriend?</p>

      <div class="choice-buttons">
        <button class="btn" id="yesBtn">Yes 💕</button>
        <button class="btn-ghost" id="noBtn">I need a moment 😊</button>
      </div>

      <div id="formWrap" style="display:none;margin-top:12px;width:100%;align-items:center;">
        <form id="askForm">
          <input type="text" id="fromName" placeholder="Your name (optional)" />
          <!-- We'll capture her answer with a hidden field -->
          <input type="hidden" id="answer" name="answer" />
          <div class="note">By submitting, your choice will be sent to the person who made this site.</div>
          <button class="btn" type="submit">Send my answer</button>
        </form>
      </div>

      <div id="thanks" style="display:none;margin-top:14px;text-align:center">
        <strong>Thank you — message sent. 🌸</strong>
      </div>

    </section>

    <footer>
      <div>Made with ❤️ — a little surprise for Phue's birthday (April 8)</div>
    </footer>
  ...
  <script>
    ...
    musicToggle.addEventListener('click', ()=>{
      if(musicPlaying){ music.pause(); musicPlaying=false; musicToggle.textContent='Play Music' }
      else{ music.play().catch(()=>{}); musicPlaying=true; musicToggle.textContent='Pause Music' }
    });
    function playMusicIfAutoAllowed(){ music.play().then(()=>{musicPlaying=true;musicToggle.textContent='Pause Music'}).catch(()=>{}); }
    ...
    yesBtn.addEventListener('click', ()=>{
      answerField.value = 'Yes — She said YES';
      showForm();
    });
    noBtn.addEventListener('click', ()=>{
      answerField.value = 'I need a moment — Not right now';
      showForm();
    });
    function showForm(){ formWrap.style.display='flex'; formWrap.scrollIntoView({behavior:'smooth'}); }
      ...
      try{
        if(window.emailjs){
          if(EMAILJS_USER_ID && !EMAILJS_USER_ID.includes('YOUR') && !EMAILJS_USER_ID.startsWith('service_')){
            ...
            console.warn('EmailJS user ID placeholder detected — emailjs.init skipped.');
          }
        } else {
    ...
    document.getElementById('askForm').addEventListener('submit', async function(e){
      ...
      if(!window.emailjs){
        console.log('EmailJS not present — attempting to load SDK dynamically.');
      ...
      if(!SERVICE_ID || !TEMPLATE_ID || SERVICE_ID.includes('YOUR') || TEMPLATE_ID.includes('YOUR')){
        ...
        console.warn('EmailJS placeholders detected — replace SERVICE_ID and TEMPLATE_ID with real IDs from your EmailJS account.');
        return;
      }
      ...
      emailjs.send(SERVICE_ID, TEMPLATE_ID, templateParams)
        .then(function(){
          ...
          alert('Sorry — something went wrong sending your answer. See console for details.');
        });
    });
